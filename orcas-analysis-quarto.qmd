...
```{r}
#| label: center means 2

library(tidyverse)
library(sf)
library(ggspatial)  # for north arrow and scale bar if needed

puget_water <- ne_download(
    scale = 50, 
  ...
  # Filter for Puget Sound
  filter(name %in% c("North Pacific Ocean")) 


ggplot() +
  geom_sf(data = puget_water,
          fill = "blue",
          color = "navy") +
  # Add original points with low alpha
  geom_sf(
    data = lines_centroids,
    alpha = 0.1,
    color = "navy"
...
# Assuming your sf object is called 'orca_points' 
# and has columns: geometry, year, and other attributes

# Calculate center means by year
center_means <- lines_centroids |> 
  filter(!st_is_empty(geometry)) |> 
  # Group by year
  group_by(year) |> 
  # Calculate center mean for each year
  summarise(
    geometry = st_combine(geometry) |> 
      st_centroid(),
    n_sightings = n()  # keeping track of number of sightings per year
  ) |> 
  ungroup()

# Create the base plot
ggplot() +
  # Add coastline or boundary polygon if you have one
  ...
  geom_sf(
    data = center_means,
    aes(size = n_sightings, color = year),
    alpha = 0.7
  ) +
  
  # Connect centers chronologically with arrows
  geom_path(
    data = center_means %>% 
      arrange(year) %>%
      st_coordinates() %>%
  ...
  # Add labels
  labs(
    title = "Changes in Orca Center Mean Location Over Time",
    subtitle = "Size indicates number of sightings per year",
    caption = "Data source: Your source here"
  ) +
  
  # Fix the aspect ratio for mapping
  coord_sf()
